...
function doEventOnStar(event, vm, position, offset) {
  ...
  const star = vm.$el.getElementsByTagName('polygon')[position]
...
describe('star-rating component', () => {
  it('should set the default props values', () => {
    const props = helpers.createVM(starRating, {}).$options.props
  ...
  it('should set the props', () => {
    ...
    const propsData = helpers.getProps(starRating, props)
  ...
  it('should round the fillLevel up to the nearest given increment', () => {
    const data = helpers.getData(starRating, { increment: 0.5, rating: 1.1 })
  ...
  it('should set the currentRating and selected Rating based on passed rating prop', () => {
    const data = helpers.getData(starRating, { rating: 2 })
  ...
  it('should set the step based on the given increment', () => {
    const data = helpers.getData(starRating, { increment: 0.1 })
    expect(data.step).toEqual(10)
  })

  it('should return the correct percentage when using rtl', () => {
    const Component = Vue.extend(starRating)
    const component = new Component()
    component.increment = 0.5

    // left-to-right
    const event = { position: 51, id: 2 }
  ...
  describe('dom events', () => {
    ...
    it('should set the currentRating to selectedRating on mouseout', () => {
      ...
      const starRating = document.getElementsByTagName('div')[1]
    ...
    it('should set the fillLevels to currentRating', () => {
      ...
      const fillLevel = vm.$children[0].$data.fillLevel
    ...
    it('should display the current rating', () => {
      vm = getViewInstance().$mount('#app')
      expect(
        document.getElementsByClassName('vue-star-rating-rating-text')[0]
      ).not.toBe(undefined)
    })
    ...
    it('should add the vue-star-rating-pointer class', () => {
      vm = getViewInstance().$mount('#app')
      const starRating = vm.$children[0].$el.getElementsByTagName('span')[0]

      expect(starRating.className).toBe(
    ...
    it('should add the textClass class to rating-text', () => {
      ...
      const ratingText = document.getElementsByClassName(
    ...
    it('should not add the vue-star-rating class to star-rating', () => {
      ...
      const ratingText = document.getElementsByClassName('vue-star-rating')[0]
    ...
    it('should add the vue-star-rating-inline class to star-rating', () => {
      ...
      const ratingText = document.getElementsByClassName('vue-star-rating')[0]
      expect(ratingText.className).toBe(
